...
Bot::Bot(int x, int y) : coordx(x), coordy(y)
{
    ...
    liste[0] = new vector("Taper", "Ordonner", "Saluer", "Cadeau", "Cadeau piege");
    ...
    ACTION_PRIM(GROUPE, 0)
        ...
        ACTION_SEC(ORDONNER, 10)
            RECURSIVE(ADD, 0) REP(0)
                CVAR(ADD, &temper, 0)
                CVAR(ADD, &mood, 0)
        ...
        ACTION_SEC(CADEAU, 12)
            RECURSIVE(ADD, 0) REP(0)
                CVAR(ADD, &sociability, 0)
                CVAR(ADD, &temper, 0)
                CVAR(ADD, &mood, 0)
                CVAR(ADD, &actionTendency, 0)
                CRECURSIVE(FACT, 1, 0) REP(1)
                    CVAR(FACT, &energy, 1)
                    CSTATIC(ADD, -1, 1)
                CSTATIC(DIV, 6, 0)
    ...
    ACTION_PRIM(SEUL, 100)
        RECURSIVE(ADD, -20) REP(0)
            CVAR(FACT, &nbBotClose, 0)

        ACTION_SEC(PROMENER, 11)
            RECURSIVE(ADD, 0) REP(0)
                CVAR(ADD, &actionTendency, 0)
                CVAR(ADD, &temper, 0)
                CVAR(ADD, &mood, 0)
                CVAR(FACT, &hunger, 0)
                CRECURSIVE(DIV, 1, 0) REP(1)
                    CVAR(ADD, &energy, 1)
                CRECURSIVE(FACT, 0, 0) REP(1)
                    CVAR(ADD, &actionTendency, 1)
                    CVAR(ADD, &sociability, 1)

        ACTION_SEC(ATTIRER, 11)
            STATIC(FACT, 3)
            RECURSIVE(ADD, 1) REP(0)
                CVAR(ADD, &actionTendency, 0)
                CRECURSIVE(DIV, 1, 0) REP (1)
                    CVAR(ADD, &energy, 1)
                CRECURSIVE(FACT, 2, 0) REP(1)
                    CRECURSIVE(FACT, 0, 1) REP(2)
                        CVAR(ADD, &temper, 2)
                        CVAR(ADD, &mood, 2)
                        CVAR(ADD, &actionTendency, 2)

        ACTION_SEC(EXPLORER, 11)
            STATIC(ADD, 6)
            RECURSIVE(ADD, 1) REP(0)
                CVAR(ADD, &actionTendency, 0)
                CVAR(ADD, &sociability, 0)
                CRECURSIVE(DIV, 1, 0) REP (1)
                    CVAR(ADD, &energy, 1)
                    CVAR(ADD, &hunger, 1)
                CRECURSIVE(FACT, 5, 0) REP(1)
                    CVAR(ADD, &temper, 1)
                    CVAR(ADD, &mood, 1)


        ACTION_SEC(SE_NOURRIR, 11)
            RECURSIVE(ADD, 2) REP(0)
                CVAR(FACT, &hunger, 0)

            REACTION(SELF, &hunger, -3)

        ACTION_SEC(DORMIR, 11)
            RECURSIVE(ADD, 2) REP(0)
                CVAR(FACT, &energy, 0)

            REACTION(SELF, &energy, +1)

    ACTION_PRIM(MIXTE, 50)
        RECURSIVE(ADD, 10) REP(0)
            CVAR(FACT, &nbBotClose, 0)

        ACTION_SEC(PARLER, 50)
            RECURSIVE(ADD, 0) REP(0)
                CVAR(ADD, &sociability, 0)
                CVAR(ADD, &temper, 0)
                CSTATIC(DIV, 2, 0)
        ...
        ACTION_SEC(CRIER, 50)
            ...
            REACTION(TARGET, &mood, -2)

        ACTION_SEC(GEINDRE, 50)
            RECURSIVE(ADD, -1) REP(0)
                CVAR(FACT, &temper, 0)
                CSTATIC(ADD, 5, 0)
                CSTATIC(FACT, 4, 0)
...
{
    delRelation(id);
    relations.push_back(new Relation(this, id));
}
...
void promener()
{
    ...
    {
        attempt = true;
        int dirx = rand() % 3 - 1, diry = rand() % 3 - 1, distance = rand() % 6 + 1, x = coordx, y = coordy;
        for(int i = 0; i<distance; i++)
        {
            ...
            {
                if(relations[j].occupied(x, y)) attempt = false;
            }
        }

        if(attempt = true) valide = true;
    }
}
